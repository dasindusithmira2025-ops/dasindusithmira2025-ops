<!--
╭───────────────────────────────────────────────────────────────────────╮
│                                                                       │
│   DASINDU SITHMIRA                                                    │
│   Founder — Corelith Technologies                                     │
│   Building Paralith, the agentic development environment              │
│                                                                       │
│   corelithtechnologies.com                                            │
│                                                                       │
╰───────────────────────────────────────────────────────────────────────╯
-->

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/hero-dark.svg"/>
  <source media="(prefers-color-scheme: light)" srcset="assets/hero-light.svg"/>
  <img src="assets/hero-dark.svg" alt="Dasindu Sithmira — Founder at Corelith Technologies. Software directed by humans, executed by intelligent agents, and verified by evidence." width="100%"/>
</picture>

<div align="center">

<br/>

<a href="https://www.corelithtechnologies.com"><b>Corelith</b></a>
&nbsp;&nbsp;·&nbsp;&nbsp;
<a href="#paralith"><b>Paralith</b></a>
&nbsp;&nbsp;·&nbsp;&nbsp;
<a href="#selected-work"><b>Work</b></a>
&nbsp;&nbsp;·&nbsp;&nbsp;
<a href="https://github.com/dasindusithmira2025-ops/paralith-updates"><b>Releases</b></a>

</div>

---

## The premise

Most development tools were designed for a world where humans manually write, review, and coordinate every part of software creation. That world is changing.

I build systems where developers define **intent**, agents execute **bounded work**, repositories become **observable environments**, and every completed task is connected to **real evidence**.

> **intent** → **mission** → **agents · tools · context** → **implementation** → **tests · diffs · artifacts** → **verified software**

The objective is not to remove developers. It is to give ambitious developers better machinery.

---

<a id="paralith"></a>

## Paralith

**The agentic development environment.** A local-first workspace for directing, observing, and verifying software work across AI agents, repositories, terminals, Git worktrees, and long-term project memory.

> AI agents should not merely generate code.
> They should operate inside a coherent engineering system.

That system should understand the repository, isolate parallel work, keep long-term context alive, surface what needs attention, and prove what was actually completed.

<table width="100%">
<tr>
<td width="50%" valign="top">

**Agent execution**

`Claude & Codex agents`
`Mission workflows`
`Role-based assignment`
`Bounded execution`
`Permission-aware actions`

</td>
<td width="50%" valign="top">

**Engineering environment**

`Multi-terminal workspaces`
`Isolated worktrees`
`Multi-project operation`
`Multi-monitor support`
`Secure filesystem`

</td>
</tr>
<tr>
<td width="50%" valign="top">

**Project intelligence**

`Persistent memory`
`Context assembly`
`Typed knowledge`
`Artifact provenance`
`Contradiction detection`

</td>
<td width="50%" valign="top">

**Verification**

`Diff inspection`
`Automated evaluations`
`Test evidence`
`Completion reports`
`Human review gates`

</td>
</tr>
</table>

<sub>Built with **Tauri** · **Rust** · **React** · **TypeScript** · **SQLite** · **Git** — source private during development, shipping through a [signed update channel](https://github.com/dasindusithmira2025-ops/paralith-updates).</sub>

---

<a id="selected-work"></a>

## Selected work

| | Project | What it is | Stack |
|:-:|:--|:--|:--|
| `01` | **[Paralith](https://www.corelithtechnologies.com)** | Agentic development infrastructure for coordinating AI coding agents, repositories, and delivery | `Tauri` `Rust` `React` `TypeScript` |
| `02` | **[PulseBoost](https://github.com/dasindusithmira2025-ops/pulseboost_cpp)** | Windows performance engineering built on measurable results and reliable rollback | `Python` `FastAPI` `Electron` |
| `03` | **TyoTrack** | Workforce operations — time tracking, approvals, auditing, offline-capable workflows | `Next.js` `NestJS` `PostgreSQL` |
| `04` | **LaunchForge** | AI-native product development, turning rough ideas into executable projects | `Next.js` `TypeScript` `Local AI` |

---

## How I build

**AI output is not evidence.** A generated answer, a green terminal, or an agent's completion claim does not prove the work is correct. Real completion requires tests, diffs, artifacts, and reproducible behaviour.

**Automation must remain observable.** A powerful system that cannot explain what it is doing becomes impossible to trust, debug, or control.

**Every dangerous action needs a recovery path.** Rollback, isolation, audit history, and explicit permissions are what make autonomy safe enough to actually use.

**The interface is part of the architecture.** A complex system should not force users to carry that complexity in their heads.

**Build systems, not demos.** A polished prototype creates the illusion of progress. A real product survives edge cases, failures, migrations, and actual users.

---

## Stack

<div align="center">

<img src="https://skillicons.dev/icons?i=ts,rust,python,cpp,cs,react,nextjs,tailwind,nodejs,nestjs,fastapi,tauri,electron,postgres,sqlite,prisma,docker,git,githubactions,linux,unreal,blender&perline=11&theme=dark" alt="TypeScript, Rust, Python, C++, C#, React, Next.js, Tailwind, Node.js, NestJS, FastAPI, Tauri, Electron, PostgreSQL, SQLite, Prisma, Docker, Git, GitHub Actions, Linux, Unreal Engine, Blender"/>

</div>

| | |
|:--|:--|
| **Languages** | TypeScript · Rust · Python · JavaScript · C++ · C# |
| **Product** | React · Next.js · Tailwind · Tauri · Electron |
| **Services** | Node.js · NestJS · FastAPI · PostgreSQL · SQLite · Prisma |
| **Infrastructure** | Git · GitHub Actions · Docker · Linux · Windows |
| **AI** | Claude · Codex · RAG · Agent memory · Evaluations |
| **Creative** | Unreal Engine · Blender · Figma |

---

## Recognition

| | |
|:--|:--|
| **2024** | APICTA — **First Runner-Up**, Brunei |
| **2025** | APICTA — **Finalist**, Chinese Taipei |
| **2025** | International Science and Engineering Fair — **Finalist**, United States |
| **Now** | Founder — Corelith Technologies |
| **Now** | Creator and Lead Product Engineer — Paralith |

---

## Now

Building a development environment where agents understand the project, parallel work stays isolated, important context survives, repository activity stays visible, humans remain in control, and completed work proves itself.

<table width="100%">
<tr>
<td width="50%" valign="top">

- Agent session recovery
- Multi-agent swarm execution
- Repository intelligence
- Long-term project memory

</td>
<td width="50%" valign="top">

- Multi-monitor agent workspaces
- Terminal-scoped Git context
- Evidence-based completion
- Signed automatic updates

</td>
</tr>
</table>

---

<div align="center">

<br/>

### Build the environment where the next generation of software is created.

<br/>

<a href="https://www.corelithtechnologies.com"><b>corelithtechnologies.com</b></a>

<br/><br/>

<sub>FOUNDER&nbsp;&nbsp;／&nbsp;&nbsp;ENGINEER&nbsp;&nbsp;／&nbsp;&nbsp;BUILDER</sub>

<br/>

</div>
